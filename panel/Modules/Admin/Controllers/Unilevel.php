...
class Unilevel extends \Modules\Common\Controllers\AdminBaseController
{
	use ResponseTrait;
    public $parser;
    ...
    public function index(){
		...
		array_push($this->data['js_entries'],
			...
			array('js_link' => base_url().'/assets/scripts/unilevel.js')
        );
        
        $this->data['js_init']      = "unilevelTable.init()
        localStorage.setItem('access_token','".$this->session->get('access_token')."');";
		$this->data['title']        = 'Unilevel Library';
		$this->data['page']         = 'Modules\Admin\Views\unilevel_code_view';
        $this->data['css_custom']   = "";
        $this->data['js_custom']    = "";
        $this->data['message']      = "";
        
        $side['side_bar'] = $this->load_sidebar(array('item_index' => 4, 'sub_index' => 0, 'page_title' => 'Dashboard', 'show_page_title' => 1, 'show_breadcrumbs' => 1, 'user_type' => $this->joshua_auth->get_session_data('user_type')));
