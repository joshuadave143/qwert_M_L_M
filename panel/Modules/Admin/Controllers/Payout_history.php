...
class Payout_history extends \Modules\Common\Controllers\AdminBaseController
{
	use ResponseTrait;
    public $parser;
    ...
    public function index(){
		...
		array_push($this->data['js_entries'],
			...
			array('js_link' => base_url().'/assets/scripts/payout_history.js')
        );
        
        $this->data['js_init']      = "payout_history.init()
        localStorage.setItem('access_token','".$this->session->get('access_token')."');";
		$this->data['title']        = 'Payout History';
		$this->data['page']         = 'Modules\Admin\Views\payout_history_view';
        $this->data['css_custom']   = "";
        $this->data['js_custom']    = "";
        $this->data['message']      = "";
