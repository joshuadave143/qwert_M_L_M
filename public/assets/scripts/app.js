...
var App = function () {
    ...
    var handleSession = function(){
        ...
        $.each(href, function( index, value ) {
            if(value == "login") has_login = true
        });
        start_time = false;
        if( window.location.href != base_url+"/" ) start_time = true
        if( has_login ) start_time = false;
        if(start_time ) runAfterThirty()//600000
    ...
    return {
        ...
        blockUI: function (options) {
            ...
            var html = '';
            if (options.iconOnly) {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><img src="'+ window.location.origin+'/assets/img/loading-spinner-grey.gif" align=""></div>';
            } else if (options.textOnly) {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            } else {    
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><img src="'+ window.location.origin+'/assets/img/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            }
